...
class CCESectionCell_Complex: UITableViewCell {
  ...
  init(primaryTitleTexts: [String], bodyTexts: [String], iconNames: [String]) {
    ...
    for (index, primaryTitleText) in primaryTitleTexts.enumerated() {
      let primaryTitle                    = CCELevelTwoTitleLabel(text: primaryTitleText, textAlignment: .left, fontSize: 18)
      let body                            = CCEBodyLabel(text: bodyTexts[index], textAlignment: .left, fontSize: 12)
  ...
  func configureUI() -> Void {
    for (index, _) in self.primaryTitles.enumerated() {
      ...
      NSLayoutConstraint.activate([
        // primary title
        self.primaryTitles[index].topAnchor.constraint(equalTo: self.topAnchor, constant: index < 1 ? p15 : unitHeight),
        self.primaryTitles[index].leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: p15),
        self.primaryTitles[index].trailingAnchor.constraint(equalTo: self.trailingAnchor, constant: p15),
        // body
        self.bodies[index].topAnchor.constraint(equalTo: self.primaryTitles[index].bottomAnchor, constant: p15),
        self.bodies[index].leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: p15),
        self.bodies[index].widthAnchor.constraint(equalToConstant: 280),
        // icon/image
        self.icons[index].topAnchor.constraint(equalTo: self.primaryTitles[index].topAnchor, constant: p15),
        self.icons[index].trailingAnchor.constraint(equalTo: self.trailingAnchor, constant: -p15),
  ...
  func set_(section cceSection: CCESection_Complex, at index: Int) -> Void {
